...
class ArticleController {
    static async getAllArticles(req, res) {
        await pool.query(allqueries.getAllArticles)
            .then((result) => {
                if (result.rows.length !== 0) {
                    res.status(200).json({
                        status: 200,
                        message: 'Retrieved all articles',
                        data: {...Object.values(result.rows) }.values(),
                    });
                } else {
                    res.status(404).json({
                        status: 404,
    ...
    static async getMyArticles(req, res) {
        await pool.query(allqueries.getMyArticles, [req.userData.id])
            .then((result) => {
                if (result.rows.length !== 0) {
    ...
    static async getArticle(req, res) {
        ...
        await pool.query(allqueries.getOneArticle, [id])
            .then((result) => {
                if (result.rows.length !== 0) {
                    return res.status(200).json({
                        status: 200,
    ...
    static async deleteArticle(req, res) {
        ...
        if (isStored.rows.length !== 0) {
            ...
            if (authorId === isStored.rows[0].authorid) {
                await pool.query(allqueries.deleteArticle, [id])
                    .then((result) => {
    ...
    static async patchArticle(req, res) {
        ...
        if (isStored.rows.length !== 0) {
            if (authorId === isStored.rows[0].id) {
                await pool.query(allqueries.updateArticle, [
                        id,
