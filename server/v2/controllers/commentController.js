...
class commentController {
  static async postComment(req, res) {
    ...
    const isThereArticle = await pool.query(allqueries.getOneArticle, [
      articleId
    ]);

    if (
      isThereArticle.rows.lenght !== 0 &&
      isThereArticle.rows[0].authorid === req.userData.id
    ) {
      await pool
        .query(allqueries.insertComment, [
          req.body.comment,
          false,
          req.userData.id,
          articleId,
